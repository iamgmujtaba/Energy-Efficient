...
public class PhoneSelector {
  ...
  public static PhoneConstants getConstants(Context context) {
    switch(getPhoneType()) {
      ...
      //  Log.w(TAG, "Phone type not recognized (" + Build.DEVICE + "), using " + (oled ? "Passion" : "Dream") + " constants");
  ...
  public static PhonePowerCalculator getCalculator(Context context) {
    switch(getPhoneType()) {
      ...
      default:
        boolean oled = hasOled();
       // Log.w(TAG, "Phone type not recognized (" + Build.DEVICE + "), using " + (oled ? "Passion" : "Dream") + " calculator");
  ...
  public static void generateComponents(Context context,
	...
	public double calculate(PowerData data) {
    	 ...
    	 // Log.e("CPU",String.valueOf(calculator.getCpuPower((CpuData)data)));
